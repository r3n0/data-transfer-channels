# data-transfer-channels


Este repositorio contiene la programación de un servidor de Node.js que permite la transferencia de datos entre nodos en un canal específico usando sockets.

## Puertos

Es importante configurar el servidor de manera que el puerto 3000 esté habilitado por el firewall.

### Inbound

|Type|Protocol|Port Range|Sources|
|---|---|---|---|
|Custom|TCP|3000|All IPv4 All IPv6|

## 1 Instalación 

Primero es importante instalar node.js

```bash
sudo apt update
sudo apt install nodejs
```

Luego es importante instalar npm
```bash
sudo apt install npm
```

A continuación se instalan las dependencias

```bash
mkdir ~/mi-servidor-servo
cd ~/mi-servidor-servo
```

```bash
npm init -y
npm install express socket.io
```

## 2 Ejecución del servidor con PM2

```bash
sudo npm install pm2 -g
pm2 start index.js --name "servidor-sockets"
```

para verificar el estado del servidor 

```bash
pm2 status
```


Detenerlo: 
```bash
pm2 stop servidor-sockets
```

Reiniciarlo: 
```bash
pm2 restart servidor-sockets
```
